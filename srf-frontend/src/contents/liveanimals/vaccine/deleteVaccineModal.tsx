...
export function DeleteVaccineModal({ vaccine, close, refresh }: DeleteVaccineModalProps) {
                ...
                <div onClick={(e) => e.stopPropagation()} className="modal relative flex flex-col bg-white w-full max-w-lg justify-center items-center rounded-2xl shadow-xl px-10 pt-12 pb-3">
                    ...
                    <form onSubmit={handleDelete} className="w-full h-full flex flex-col items-center justify-center">
                        <p className="text-xl text-center">
                            Deseja realmente excluir a aplicação da vacina <span className="font-bold">{vaccine.vaccineName}</span> do tipo <span className="font-bold">{vaccine.vaccineTypeName}</span> aplicada no animal <span className="font-bold">{vaccine.liveAnimalName}</span>?
                        </p>
