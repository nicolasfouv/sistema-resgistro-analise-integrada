...
export function DeleteEggCystAnalysisModal({ eggCystAnalysis, close, refresh }: DeleteEggCystAnalysisModalProps) {
    ...
    async function handleDelete(e: React.FormEvent) {
        ...
        setLoading(true);
        try {
            await deleteEggCystAnalysis(eggCystAnalysis.id);
            refresh();
            close();
        } catch (error: any) {
            console.error(error);
                ...
                <div onClick={(e) => e.stopPropagation()} className="modal relative flex flex-col bg-white w-full max-w-lg justify-center items-center rounded-2xl shadow-xl px-10 pt-12 pb-3">
                    ...
                    <form onSubmit={handleDelete} className="w-full h-full flex flex-col items-center justify-center">
                        <p className="text-xl text-center">
                            Deseja realmente excluir a análise de ovos/cistos da espécie <span className="font-bold">{eggCystAnalysis.eggCystSpecieName}</span> do animal <span className="font-bold">{eggCystAnalysis.liveAnimalName}</span>?
                        </p>
