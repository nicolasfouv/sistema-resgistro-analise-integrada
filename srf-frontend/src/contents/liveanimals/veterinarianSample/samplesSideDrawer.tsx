...
export function SamplesSideDrawer({ filters, onClose }: SamplesSideDrawerProps) {
        ...
        getVeterinarianSamples()
            .then(all => {
                const filtered = all
                    ...
                    .filter(s => {
                        if (filters.veterinarianVisitId && s.veterinarianVisitId !== filters.veterinarianVisitId) return false;
                        if (filters.liveAnimalId && s.liveAnimalId !== filters.liveAnimalId) return false;
                        return true;
                    });
                setSamples(filtered);
            })
    ...
    const pageUrl = `/animaisvivos/veterinario/amostras-av?filters=${encodeURIComponent(JSON.stringify(pageFilters))}`;
